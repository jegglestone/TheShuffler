namespace ShufflerLibrary.Tests.Unit
{
    ...
    {
        [Test]
        public void WhenMdBkByOrTMThenMoveAfterShufflingTM()
        {
            /*
            ...
            Assert.That(sentenceWithPrenNNPastUnit.Texts[6].pe_text, Is.EqualTo("in"));  //TM2
            Assert.That(sentenceWithPrenNNPastUnit.Texts[7].pe_text, Is.EqualTo("2011"));
            Assert.That(sentenceWithPrenNNPastUnit.Texts[8].pe_text, Is.EqualTo("per"));//TM1
            Assert.That(sentenceWithPrenNNPastUnit.Texts[9].pe_text, Is.EqualTo("month"));
            //PAST
            Assert.That(sentenceWithPrenNNPastUnit.Texts[10].pe_text, Is.EqualTo("added")); //PAST
            Assert.That(sentenceWithPrenNNPastUnit.Texts[11].pe_text, Is.EqualTo("jobs"));   //NN

            Assert.That(sentenceWithPrenNNPastUnit.Texts[12].pe_text, Is.EqualTo(" . "));
        }

        [Test]
        public void WhenMdBkByOrTMThenMoveAfterShufflingMD()
        {
            /*
             1.	Search for the structure ‘PREN + NN + PAST’ or ‘NN + PAST’
             2.	If found, search for ‘BKby’, MD or TM until reaching VB/NBKP/BKP. For example, the following sentence -
            ...
            var sentenceWithPrenNNPastUnit = new Sentence()
            {
                Texts = new List<Text>()
                {
                    ...
                    new Text() {pe_tag = "NN", pe_text = "jobs"},
                    new Text() {pe_tag = "PAST", pe_text = "added"},
                    new Text() {pe_tag = "MD1", pe_text = "per"},
                    new Text() {pe_tag = null, pe_text = "month"},
            ...
            Assert.That(sentenceWithPrenNNPastUnit.Texts[6].pe_text, Is.EqualTo("in"));  //MD2
            Assert.That(sentenceWithPrenNNPastUnit.Texts[7].pe_text, Is.EqualTo("2011"));
            Assert.That(sentenceWithPrenNNPastUnit.Texts[8].pe_text, Is.EqualTo("per"));//MD1
            Assert.That(sentenceWithPrenNNPastUnit.Texts[9].pe_text, Is.EqualTo("month"));
            //PAST
            Assert.That(sentenceWithPrenNNPastUnit.Texts[10].pe_text, Is.EqualTo("added")); //PAST
            Assert.That(sentenceWithPrenNNPastUnit.Texts[11].pe_text, Is.EqualTo("jobs"));   //NN
